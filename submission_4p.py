"""
4-parameter Qwen-style adder (PyTorch).

Four independent parameters control the entire model:
  c       (1)  embedding scale  — also derives norm weights and gate[1]
  v       (1)  attention value strength
  g       (1)  carry detection threshold
  s       (1)  carry pathway amplitude

Derived (0 additional params):
  norm[0] = 0.1·c / √2        (decode curvature)
  norm[1] = -c / (50·√2)      (digit scale for output)
  gate[1] = 128·c              (digit-proportional gate component)
  φ = (2π/19)·10.3            (RoPE positional encoding constant, free per rules)
"""
...
PEAK_EPS = 0.3
PHI = OMEGA * (10.0 + PEAK_EPS)  # fixed positional encoding constant

EMBED_CONST = 1000.0  # default value for c
...
class AdderModel(nn.Module):
    def __init__(self):
        ...
        self.v = nn.Parameter(torch.zeros(1))       # attention value strength
        self.g = nn.Parameter(torch.zeros(1))       # carry detection threshold
        self.s = nn.Parameter(torch.zeros(1))       # carry pathway amplitude
    ...
    def _gate_weight(self):
        c = self.c[0]
        return torch.stack([self.g[0], 128.0 * c])

    # ── projections (inline, no submodules) ─────────────────────────
    def _q_proj(self, x):
        # fixed φ — positional encoding constant
    ...
    def _v_proj(self, x):
        return torch.stack([x[..., 1] * self.v[0],
    ...
    def forward(self, tokens: torch.Tensor) -> torch.Tensor:
        ...
        # pre-attn norm → attention → residual
        hn = _unit_rms_norm(h)
        h = h + self._attention(hn, mask)

        # post-attn norm → MLP → residual
        hn = _unit_rms_norm(h)
        h = h + self._mlp(hn)

        # final norm → logits via tied embedding
...
def _init_weights(model: AdderModel) -> None:
    c = EMBED_CONST
    with torch.no_grad():
        model.c.copy_(torch.tensor([c]))
        model.v.copy_(torch.tensor([-22.0 * c / CONST_NORM]))
        model.g.copy_(torch.tensor([256.0 * (-94.0) / (CONST_NORM * CONST_NORM)]))
        model.s.copy_(torch.tensor([100.0 * CONST_NORM / (256.0 * CONST_NORM)]))
...
def build_model():
    ...
    metadata = {
        ...
        "architecture": "4 parameter",
        "tricks": [
            "RoPE period-19 positional encoding",
            "phase-tied Q projection (hardcoded φ, positional constant)",
            "tied embedding (single scalar c → full vocab table)",
            "derived norm weights from c",
            "derived gate component from c",
            "hardcoded carry pathway constant",
        ],
    }
